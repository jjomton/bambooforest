# [최종 보고서] 휴먼센터 대나무숲 익명 고충 건의 시스템 신규 구축 결과 보고서
...
### 1.1 문제 정의 및 목표
* **배경 및 문제 정의:** 
  학원 내 수강생들이 장시간 학습을 진행하면서 겪는 강의실 환경(에어컨 온도, 휴게실 혼잡, 비품 부족 등)에 대한 고충과 건의사항을 직접 건의하기에 심리적 장벽이 컸으며, 이에 대한 수렴과 피드백 프로세스가 존재하지 않아 소통의 불통 문제가 있었습니다.
* **프로젝트 목표:**
  완벽한 **익명성**을 보장하여 수강생들이 심리적 부담 없이 민원을 제출할 수 있는 소통 창구를 신규 개설하고, 학원 운영진(관리자)이 이에 대해 수용 여부 및 처리 현황을 투명하게 공시하며, 비속어 필터링과 자동 블라인드 정책을 통해 커뮤니티의 자정 능력을 갖춘 웹 애플리케이션을 최초 구축합니다.

### 1.2 팀원 및 역할 (R&R)
5인 협업 개발팀을 구성하여 각자의 전문 영역을 살려 신규 구축 개발을 완료하였습니다.

| 성명 | 담당 역할 및 성과 |
|:---:|:---|
| **변영진** | 개발 총괄, Supabase 보안 인프라 설계, RLS 정책 수립, Notion API 통합 및 배포 자동화 |
| **김종록** | 데이터베이스 스키마 설계, 실시간 댓글 및 계층형 대댓글(답글) 시스템 구현 |
| **김혜라** | 모더레이션 화면(신고 현황판) UX 설계, 클라이언트 비속어 필터링 및 자동 블라인드 프론트 흐름 구현 |
| **안민영** | CI/CD 환경 가이드 및 배포 자동화 검증 |
| **임장빈** | 테스트 총괄 |

### 1.3 개발 일정
* **1단계 (요구사항 정의 및 스키마 설계):** 기능 정의서 작성 및 Supabase DB 관계 모델링, 초기 와이어프레임 설계
* **2단계 (기본 기능 구현):** 익명 게시판 구축, 공감도 평점 투표 시스템 개발 및 프론트엔드 기본 UI 구현
* **3단계 (고급 기능 추가):** 관리자 모더레이션 대시보드, 계층형 대댓글 시스템, 정규식 기반 비속어 필터링 도입
* **4단계 (통합 및 안정화):** 기능 병합, 통합 테스트 수행, 자동 배포(Netlify) 파이프라인 구축 및 최종 검증
...
* **API & Automation:** Node.js CLI Script, Notion API (Troubleshooting 연동)
* **CI/CD:** GitHub Actions (PR & Push 자동 빌드 검증), Netlify (자동 배포)
...
이에 변영진 님이 설계한 **섀도 매핑 테이블(Shadow Mapping Table) 패턴**을 적용하여 이 문제를 해결하였습니다.
...
### 4.1 통합 테스트 시나리오
임장빈 테스트 총괄의 주도 하에 시스템의 정상 작동 여부를 파악하기 위해 다음 7단계 시나리오를 정의하여 전수 테스트를 실행하였습니다.
...
### 5.1 트러블 슈팅 핵심 이력
신규 개발 및 환경 구축 과정에서 직면했던 4대 기술적 장애와 해결 원인을 다음과 같이 요약합니다.
...
2. **Git Author 식별자 미지정 장애 (`Author identity unknown`)**
   * **원인:** 개발 환경 최초 구축 중 Git 로컬 설정에 작성자 이메일과 이름이 지정되지 않아 머지 커밋 작성이 차단됨.
   * **조치:** `git config` 명령어로 변영진 님의 식별 정보(`byj1230@gmail.com`, `jjomton`)를 명확히 등록하여 협업 브랜치 병합을 완결함.
...
4. **Vite 빌드 타입 불일치 에러 (`TS2741: Property 'is_blinded' is missing`)**
   * **원인:** 신규 기능 개발 과정에서 댓글 Mock 데이터 객체에 필수 속성인 `is_blinded` 값이 누락되어 컴파일 에러 발생.
...
## 06. 프로젝트 회고
신규 구축 프로젝트로서 5명의 개발팀이 깃플로우(Git Flow) 협업 가이드라인을 명확히 준수하고, 백엔드 서버 없이도 완벽한 익명 보안 설계(BaaS RLS + Shadow Table)를 최초 도입해 보는 뜻깊은 경험이었습니다. 
